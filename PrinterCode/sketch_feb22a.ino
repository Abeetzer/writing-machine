...
Stepper MSP(stepsPerRevolution, 2,3,4,5);
...
float StepsPerMillimeterX = 40;
float StepsPerMillimeterY = 40;

float Xmin = 0;
float Xmax = 108;//can be a 128 if the pen holder is changed
float Ymin = 0;
float Ymax = 138;
float Zmin = 0;
float Zmax = 1;
...
void loop() 
{
 ...
 while (1) {
     ...
     if(analogRead(X_pin) == 1023){
      ...
      if(analogRead(Y_pin) == 0){myStepperY.step(-40);}
        if(analogRead(Y_pin) == 1023){myStepperY.step(40);}
          if(digitalRead(SW_pin) == LOW){MSP.step(-100);}
               if(digitalRead(Spin) == HIGH){MSP.step(50);}     
